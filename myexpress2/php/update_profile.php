...
if (isset($name) && (!empty($name))){
    $sql = "UPDATE DRIVER SET NAME = '$name' WHERE EMAIL = '$email'";
}
if (isset($password) && (!empty($password))){
    $sql = "UPDATE DRIVER SET PASSWORD = sha1($password) WHERE EMAIL = '$email'";
}
if (isset($radius)&& (!empty($radius))){
    $sql = "UPDATE DRIVER SET RADIUS = '$radius' WHERE EMAIL = '$email'";
}
if (isset($phone) && (!empty($phone))){
    $sql = "UPDATE DRIVER SET PHONE = '$phone' WHERE EMAIL = '$email'";
}
